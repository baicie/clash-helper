...
13. 连续倒计时自动选择最近完成项目，调起 Android 系统倒计时，并在结束后切换到下一个
14. 当前村庄支持批量创建未来 24 小时内的系统闹钟
15. 默认跳过 22:00–10:00 休息时段，并支持在设置页调整
16. 首页只展示村庄列表，通过右上角加号进入导入页
17. 支持清理已响闹钟的应用记录，并跳转系统时钟处理残留条目
18. 同 tag 更新 JSON 时自动对账系统闹钟：保留未变化、移除旧项目、创建新增项目
```
...
连续倒计时模式会为当前最近完成的项目创建 Android 系统倒计时。App 进程仍在运行时，当前项目归零后会自动选择并启动下一个；如果厂商系统完全终止 App，则在下次打开 App 时继续接力，期间每个项目原有的本地通知仍按排期触发。

系统闹钟由手机时钟应用持有。应用可以清理自身保存的闹钟创建记录，但 Android 没有跨厂商可靠删除历史闹钟的标准接口；清理后会提供系统时钟入口，供用户删除荣耀或小米时钟中保留的已停用条目。

执行“批量创建闹钟”后，该村庄会开启系统闹钟自动更新。再次导入相同 tag 的 JSON 时，应用会保留完成时间未变化的闹钟，请求移除已经消失或完成时间变化的旧闹钟，并为新增且符合 24 小时与休息时段规则的项目创建闹钟。荣耀或小米系统不支持标准移除 Intent 时，会在更新结果中计入“需手动处理”。
